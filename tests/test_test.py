...
def test_nested_diff():
    ...
    assert str(diff_build(file1, file2)) == (
        open('./tests/fixtures/diff.txt').read())
...
def test_compare_simple():
    ...
    assert generate_diff(file1, file2) == (
        open('./tests/fixtures/test1.txt').read())


def test_compare2_stylish():  # добавить yml
    nested1j = './tests/fixtures/nested1.json'
    nested2j = './tests/fixtures/nested2.json'
    assert generate_diff(nested1j, nested2j) == (
        open('./tests/fixtures/stylish_test.txt').read())


def test_compare_plain():
    nested1j = './tests/fixtures/nested1.json'
    nested2j = './tests/fixtures/nested2.json'
    assert generate_diff(nested1j, nested2j, 'plain') == (
        open('./tests/fixtures/plain_test.txt').read())


def test_compare_jsonq():
    nested1j = './tests/fixtures/nested1.json'
    nested2j = './tests/fixtures/nested2.json'
    assert generate_diff(nested1j, nested2j, 'json') == (
        open('./tests/fixtures/jsonn.json').read())


def test_parsing():
    assert parsing('./tests/fixtures/file1.json') == (
        {'host': 'hexlet.io', 'timeout': 50, 'proxy': '123.234.53.22',
         'follow': False})
    assert parsing('./tests/fixtures/file2.yml') == (
